...
namespace SQLTool.Common.PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        //Return DataTable
        public DataTable FetchDt<T>(string parameter, string sql, params object[] args)
        {
            return QueryDt<T>(parameter, sql, args);
        }
        ...
        //Return DataTable
        public DataTable QueryDt<T>(string parameter, string sql, params object[] args)
        {
            DataTable dt = new DataTable();
            var pd = PocoData.ForType(typeof(T));
            var tableName = EscapeTableName(pd.TableInfo.TableName);
            if (EnableAutoSelect)
                sql = "select " + parameter + " from " + tableName + " " + sql;
        ...
        {
            var pd = PocoData.ForType(poco.GetType());
            return Update(pd.TableInfo.TableName, primaryKeyName, poco, primaryKeyValue, null);
        }
